...
router.all("/api/push/:pushToken", async (request, response) => {
    try {
        ...
        // Calculate uptime
        let uptimeCalculator = await FoxticUptimeCalculator.getUptimeCalculator(monitor.id);
...
router.get("/api/badge/:id/uptime/:duration?", cache("5 minutes"), async (request, response) => {
    ...
    try {
        ...
        if (!publicMonitor) {
            ...
            const uptimeCalculator = await FoxticUptimeCalculator.getUptimeCalculator(requestedMonitorId);
