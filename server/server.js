...
// Exit FoxTic immediately if the Node.js version is banned
...
if (!process.env.FOXTIC_HIDE_LOG) {
    process.env.FOXTIC_HIDE_LOG = "debug_settings,debug_incrementalvacuum,debug_db,debug_monitor,debug_foxtic-calc";
}
...
log.debug("server", "Inside Container: " + (process.env.FOXTIC_IS_CONTAINER === "1"));

if (process.env.FOXTIC_WS_ORIGIN_CHECK === "bypass") {
    log.warn("server", "WebSocket Origin Check: " + process.env.FOXTIC_WS_ORIGIN_CHECK);
}

const checkVersion = require("./check-version");
log.info("server", "FoxTic Version: " + checkVersion.version);
...
const disableFrameSameOrigin = !!process.env.FOXTIC_DISABLE_FRAME_SAMEORIGIN || args["disable-frame-sameorigin"] || false;
const cloudflaredToken = args["cloudflared-token"] || process.env.FOXTIC_CLOUDFLARED_TOKEN || undefined;
        ...
        socket.on("setup", async (username, password, callback) => {
            try {
                ...
                if ((await R.knex("user").count("id as count").first()).count !== 0) {
                    throw new Error("FoxTic has been initialized. If you want to run setup again, please delete the database.");
                }
...
async function afterLogin(socket, user) {
    ...
    await Promise.allSettled([
        ...
        client.sendAPIKeyList(socket),
        client.sendRemoteBrowserList(socket),
...
async function initDatabase(testMode = false) {
    ...
    // If there is no record in user table, it is a new FoxTic instance, need to setup
    if ((await R.knex("user").count("id as count").first()).count === 0) {
...
let unexpectedErrorHandler = (error, promise) => {
    console.trace(error);
    FoxTicServer.errorLog(error, false);
    console.error("If you keep encountering errors, please contact your system administrator.");
};
