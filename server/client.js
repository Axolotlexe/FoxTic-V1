...
async function sendInfo(socket, hideVersion = false) {
    ...
    if (!hideVersion) {
        ...
        isContainer = (process.env.FOXTIC_IS_CONTAINER === "1" || process.env.UPTIME_KUMA_IS_CONTAINER === "1");
...
async function sendMonitorTypeList(socket) {
    const result = Object.entries(FoxTicServer.monitorTypeList).map(([ key, type ]) => {
        return [ key, {
