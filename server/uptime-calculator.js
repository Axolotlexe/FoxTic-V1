 ...
 */
class FoxticUptimeCalculator {
    /**
     * @private
     * @type {{string:FoxticUptimeCalculator}}
     */
    static list = {};
     ...
     * @returns {Promise<FoxticUptimeCalculator>} FoxticUptimeCalculator
     */
    static async getUptimeCalculator(monitorID) {
        ...
        if (!FoxticUptimeCalculator.list[monitorID]) {
            FoxticUptimeCalculator.list[monitorID] = new FoxticUptimeCalculator();
            await FoxticUptimeCalculator.list[monitorID].init(monitorID);
        }
        return FoxticUptimeCalculator.list[monitorID];
    }
    ...
    static async remove(monitorID) {
        delete FoxticUptimeCalculator.list[monitorID];
    }
    ...
    constructor() {
        if (process.env.TEST_BACKEND) {
            ...
            // Only for testing
            this.getCurrentDate = () => {
                if (FoxticUptimeCalculator.currentDate) {
                    return FoxticUptimeCalculator.currentDate;
                } else {
                    return dayjs.utc();
                }
            };
    ...
    async update(status, ping = 0, date) {
        ...
        if (flatStatus === DOWN && ping > 0) {
            log.debug("foxtic-calc", "The ping is not effective when the status is DOWN");
        }
        ...
        if (process.env.TEST_BACKEND) {
            log.debug("foxtic-calc", "Skip storing data in test mode");
            return date;
        }
        ...
        if (!this.migrationMode) {
            ...
            log.debug("foxtic-calc", "Remove old data");
