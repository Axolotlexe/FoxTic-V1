    ...
    <div>
        ...
        <div class="my-4">
            ...
            <div>
                ...
                <div v-if="$root.userTheme === 'auto'" class="system-preference-info mt-2">
                    <font-awesome-icon :icon="['fas', 'info-circle']" class="text-info me-1" />
                    <span>{{ $t("Currently using") }}: {{ $root.system === 'dark' ? $t("Dark") : $t("Light") }} {{ $t("theme based on system preference") }}</span>
                </div>
            </div>
        </div>
        <div class="my-4">
...
.system-preference-info {
    ...
    padding: 0.5rem;
    border-radius: 5px;
    background-color: rgba(23, 162, 184, 0.05); // Utiliser une couleur info directement
    border-left: 3px solid #17a2b8; // Couleur info (bleu)
