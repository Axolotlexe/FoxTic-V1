...
class Logger {
    constructor() {
        ...
        if (typeof process !== "undefined" && (process.env.FOXTIC_HIDE_LOG || process.env.UPTIME_KUMA_HIDE_LOG)) {
            const list = (process.env.FOXTIC_HIDE_LOG || process.env.UPTIME_KUMA_HIDE_LOG).split(",").map(v => v.toLowerCase());
            ...
            this.debug("server", "FOXTIC_HIDE_LOG is set");
